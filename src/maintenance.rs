...
pub(crate) async fn maintain_planes(cookies: &str) -> Vec<String> {
    let response = get_response_text("https://www.airlinemanager.com/maint_plan.php", cookies).await;

    let mut maintained_planes: Vec<String> = Vec::new();

    for i in 1..=300 {
        let plane_name = get_text_by_selector(&response, &format!("#acListView > div:nth-child({i}) > div.col-sm-4.text-center.text-secondary.s-text"))
        ...
        if hours_to_check < 15 {
            ...
            // #controls29418557
            let plane_id = get_attr_by_selector(&response, &format!("#acListView > div:nth-child({i}) > div.controls"), "id").await.replace("controls", "");
            ...
            println!("plane {plane_name} needs maintainance")
        }
    }
